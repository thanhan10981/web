...
<body> 
    <div class="container">
            ...
            <nav>
                <ul>
                    ...
                    <li class="submenu">
                        ...
                        <ul>  
                            <li><a href="#"><i class="fa-solid fa-screwdriver-wrench"></i> Đang Chiếu</a></li>
        ...
        <div class="name-home">
            <h3><i class="fa-solid fa-wrench"></i> SỬA THÔNG TIN PHIM ĐANG CHIẾU VÀ SẮP CHIẾU</h3>
        ...
        <?php
            ...
            if (!empty($keyword)) {
                $search_sql = "AND (maphim LIKE '%$keyword%' OR theloai LIKE '%$keyword%')";
            } else {
                $search_sql = '';
            }

            $sql = "SELECT * FROM phim_dang_chieu WHERE 1 $search_sql LIMIT $offset, $rows_per_page";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                     echo "<table border='1'>";
                    echo "<tr><th>Mã phim</th><th>Hình ảnh</th><th>Thể loại</th><th>Ngày phát hành</th><th>Mô tả</th><th>Hành động</th></tr>";
                    while($row = $result->fetch_assoc()) {
                        echo "<tr>";
                        echo "<th>" . $row["maphim"] . "</th>";
                        echo "<th><img src='" . $row["anh"] . "' alt='Hình ảnh phim'></th>";
                        echo "<th>" . $row["theloai"] . "</th>";
                        echo "<th>" . $row["ngayphathanh"] . "</th>";
                        echo "<td>" . $row["mota"] . "</td>";
                        echo "<th><a href='sua_phim.php?maphim=" . $row["maphim"] . "'> <i class='fa-solid fa-screwdriver-wrench'> </i> Sửa</a>
                        <a href='../php/xu_ly_xoaphim.php?maphim=" . $row["maphim"] . "'> <i class='fa-solid fa-trash'> </i> Xóa</a>
                        <a href='../php/them_phim.php '> <i class='fa-solid fa-plus'></i> Thêm</a> </th>"; 
                        echo "</tr>";
                }
                echo "</table>";
            } else {
